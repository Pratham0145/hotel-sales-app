...
function buildComparisonEmailHtml({ current, previous }) {
  ...
  <div style="max-width:560px;margin:0 auto;font-family:sans-serif;color:#221C18;">
    ...
    <div style="border:1px solid #ddd;border-top:none;border-radius:0 0 8px 8px;padding:10px 22px 22px;">
      ...
      ${sideBySide('Total Sale', current.report.totalSale, previous.report.totalSale)}
      ${sideBySide('JP Nagar Difference', current.report.jpDiff, previous.report.jpDiff)}
      ${sideBySide('Truck Difference', current.report.truckDiff, previous.report.truckDiff)}
      ${sideBySide('Total Difference', current.report.totalDiff, previous.report.totalDiff)}
      ${sideBySide('Combined Collections', current.amount.combinedTotal, previous.amount.combinedTotal)}
